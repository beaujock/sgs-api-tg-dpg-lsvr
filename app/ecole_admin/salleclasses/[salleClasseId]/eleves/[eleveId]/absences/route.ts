...
import { getEleveAbsences } from "@/factories/eleveFactory";
...
export async function GET(request:NextRequest, { params }: { params: Promise<{salleClasseId: string, eleveId:string }> }){
    try {
        ...
        const eleveAbsences = await getEleveAbsences(eleveID);
        return NextResponse.json({eleveAbsences: eleveAbsences}); 
    }
    catch(error:any){
