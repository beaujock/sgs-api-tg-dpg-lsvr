...
import { getEleveOverview } from "@/factories/eleveFactory";
...
export async function GET(request:NextRequest, { params }: { params: Promise<{salleClasseId: string, eleveId:string }> }){
    try {
        ...
        const eleveOverview = await getEleveOverview(eleveID);
        return NextResponse.json({eleveOverview : eleveOverview}); 
    }
    catch(error:any){
